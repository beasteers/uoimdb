...
class ImageProcessor(object):
    def __init__(self, imdb):
        ...
        bgsub = lambda method: imdb.pipeline().use_window().single_bgsub3(method=method).scale().clip().invert()

        # all filters
        self.filters = odict([
            # imdb.pipeline().use_window().single_bgsub2(method='mean')
    ...
    def process_image(self, filename, filter=None, check_cache=True, save_cache=False):
